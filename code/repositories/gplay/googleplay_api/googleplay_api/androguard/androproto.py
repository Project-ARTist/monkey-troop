...
def find_mergeFrom(dvm, cn):
    l = [m for m in dvm.get_methods_class(cn) if m.get_name() == "mergeFrom" and not m.get_descriptor().endswith("MessageMicro;")]
    if (len(l) != 1):
...
def index_basic_blocks(dvm, vma, cn):
    ...
    l = [bb for bb in bbs if bb.get_instructions()[-1].get_name() == "sparse-switch"]
    if (len(l) != 1):
        return {} # TODO
...
proto_classes = [c for c in dvm.get_classes() if "MessageMicro;" in c.get_superclassname()]
if (len(proto_classes) == 0):
    print("Unable to find protobuf micro classes.")
    sys.exit(0)

proto_class_names = [c.get_name() for c in proto_classes]
...
for pcn in proto_class_names:
    ...
    d = {}
    for (k, bb) in list(index_basic_blocks(dvm, vma, pcn).items()):
...
def print_proto(d, parent = (), indent=0):
    """Display all protos"""
    for m, sd in sorted(list(d.items()), cmp=lambda x,y: cmp(x[0],y[0])):
...
def print_message(name, sd, parent, indent, title="message", extras=[]):
    ...
    groups = [field for (field, typ, _) in list(infos.values()) if typ == 'group']
    print_proto(dict([(k, m) for (k, m) in list(sd.items()) if k not in groups]), full_name_l, i)

    for k, info in sorted(list(infos.items()), cmp=lambda x,y: cmp(x[0],y[0])):
        field, typ, rule = info
